...
function draw2D(ctx,text, score, highest, level,lives,ai_lives) {
  ctx.clearRect(0, 0, 800, 540); // Clear <hud>
   ...
   ctx.fillStyle = "rgba(22, 46, 100,0.6)";
   ctx.fillRect(canvas.width-198, 0, 94 + 14*score.toString().length, 30);
//   if ( displayHighest){
//   		ctx.fillStyle = "rgba(22, 46, 100,0.6)";
//   		ctx.fillRect(canvas.width-198, 57, 114 + 14*highest.toString().length, 30);
//	}
...
//  if (displayHighest)
//  	ctx.fillText("Highest Score!", canvas.width - 490, 24);
  
 // 	ctx.fillText("Highest: " + highest, canvas.width - 193, 78);

//  	ctx.fillText("Highest: " + highest, canvas.width - 450, 24);
...
function drawwelcome()
{
	...
	*/
	...
	wtx.font = 'bold 60px Comic Sans MS';
	wtx.fillStyle = 'rgba(255, 255, 255, 0.5)'; // Set white to the color of letters
	wtx.fillText("CURVEBALL", 220, 270);
	wtx.font = 'bold 25px Helvetica';
