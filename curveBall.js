...
function handleMouseMove(event) {
	...
	if (X > 708) deltaX = 300;
	else if (X < 108) deltaX = -300;
	else deltaX = X - 408;

	if (Y > 488) deltaY = -210;
	else if (Y < 68) deltaY = 210;
	else deltaY = 278 - Y;
...
window.onload = function init(){
	...
	gl.bufferData(gl.ARRAY_BUFFER, ballBufArrs.nArr, gl.STATIC_DRAW);
	...
	/*welcome_image = new Image();
	welcome_image.src = "spiral.jpg";*/
